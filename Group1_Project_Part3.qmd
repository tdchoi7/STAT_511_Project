...
> There seems to be the most non-normality in adult_mort and alcohol

##### adult_mort
```{r}

life_exp_2013 <- life_exp_2013 %>%
  mutate(adult_mort_trnsfrm = adult_mort^(1/2))

life_exp_2013 <- life_exp_2013 %>%
  mutate(longevity_trnsfrm = longevity^(2.1))

adultmort_lm <- lm(longevity_trnsfrm ~ adult_mort_trnsfrm, life_exp_2013)

adultmort_aug_lm <- augment(adultmort_lm)

ggplot(data = adultmort_aug_lm,
               aes(sample = .resid)) +    
        geom_qq_line() +
        geom_qq() +
        labs(title = glue('QQ of Residuals vs adult_mort'),
                  x = 'Theoretical',
                  y = "Sample") +
        theme_minimal()
...
print(glue('The p value for the transformed longevity vs adult_mort is {signif(tidy(summary(adultmort_lm))[2,5], 3)}'))
...
life_exp_2013 <- life_exp_2013 %>%
  mutate(alcohol_trnsfrm = alcohol^(-1))

life_exp_2013 <- life_exp_2013 %>%
  mutate(longevity_trnsfrm = longevity^2.587)

alcohol_lm <- lm(longevity_trnsfrm ~ alcohol_trnsfrm, life_exp_2013)
...
ggplot(data = alcohol_aug_lm,
        ...
        geom_qq_line() +
        geom_qq() +
        labs(title = glue('QQ of Residuals vs alcohol'),
                  x = 'Theoretical',
                  y = "Sample") +
        theme_minimal()
...
print(glue('The p value for the transformed longevity vs alcohol is {signif(tidy(summary(adultmort_lm))[2,5], 3)}'))

```

> perc_exp, hep_b, measles, bmi, u5_deaths, polio, tot_exp, dipth seem to have minor issues when looking at the QQ plot.


> There seems to be fanning in measles, u5_deaths
> - aids
> - pop
> - thin_1_19
> - thin_5_9
...
```{r}
continent.lm <- lm(longevity ~ life_exp_2013$continent.f, data=life_exp_2013)
...
life_exp_2013$continent.f <- factor(life_exp_2013$continent)
is.factor(life_exp_2013$continent.f)
```
