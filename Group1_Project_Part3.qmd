...
# dataframe of continuous columns
# other numerical variables seem to be counts (some based on per 1000, but still count nonetheless)
life_exp_2013_tfm_cont <- life_exp_2013[,c('longevity_squared',
                                           'alcohol',
                                           'perc_exp_trnsfrm',
                                           'bmi',
                                           'tot_exp',
...
# fit the model and the augment
adult_mort_lm <- lm(longevity_squared ~ adult_mort_trnsfrm, life_exp_2013)
...
# fit the model and the augment
inf_death_lm <- lm(longevity_squared ~ inf_death_trnsfrm, life_exp_2013)
...
# fit the model and the augment
hep_b_lm <- lm(longevity_squared ~ hep_b_trnsfrm, life_exp_2013)
...
# transform the independent variable
life_exp_2013 <- life_exp_2013 %>%
  mutate(measles_trnsfrm = measles^(4))
...
# fit the model and the augment
measles_lm <- lm(longevity_squared ~ measles_trnsfrm, life_exp_2013)
...
# transform the independent variable
life_exp_2013 <- life_exp_2013 %>%
  mutate(u5_deaths_trnsfrm = u5_deaths^(1/3))
...
# fit the model and the augment
u5_deaths_lm <- lm(longevity_squared ~ u5_deaths_trnsfrm, life_exp_2013)
...
# fit the model and the augment
polio_lm <- lm(longevity_squared ~ polio_trnsfrm, life_exp_2013)
...
# fit the model and the augment
dipth_lm <- lm(longevity_squared ~ dipth_trnsfrm, life_exp_2013)
...
# fit the model and the augment
pop_lm <- lm(longevity_squared ~ pop_trnsfrm, life_exp_2013)
...
```
> The p value for transformed longevity vs the transformed pop is `r signif(tidy(summary(pop_lm))[2,5], 3)`
...
life_exp_2013_tfm <- cbind(life_exp_2013_tfm_cont, life_exp_2013_tfm_disc)
