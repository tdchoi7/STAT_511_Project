...
Forward_model <- lm(life_exp_2013_response ~ ., data = life_exp_pt4)
ols_step_forward_p(Forward_model)

# Stepwise Backward Regression
Back_model <- lm(life_exp_2013_response ~ ., data = life_exp_pt4)
ols_step_backward_p(Back_model)

# Best Subset Regression
models <- regsubsets(life_exp_2013_response~., data = life_exp_pt4, nvmax = 5)
summary(models)
```
...
> The adult mortality and alcohol trnsfrm variables will be a possible pairwise interaction since they are present in most of the data and have high R2 values.
...
# Reduced Model
Reduced <- lm(life_exp_2013_response ~ adult_mort*alcohol_trnsfrm, data = life_exp_pt4)

anova(Reduced,lm_lfexp_pt4)
```
> Since the F statistic (9.7) is greater than the p-value(<2.2e-16), adult mortality and alcohol trnsfrn shouldn't be included to the model.
