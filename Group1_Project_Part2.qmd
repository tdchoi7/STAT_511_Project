...
> The thinness of 1-19 and 5-9 years could be combined so that there is just one graph showing prevelance of thinness for ages 5-19.
...
> From the variables we chose to include, hepatitis B, population, and gdp were missing in larger percentages. This is due to a country not producing enough information regarding hepatitis B during that year.
...
> We will fill the blank spaces with NA values so we can account for them.
...
ggplot(life_exp, aes(x = longevity)) +
  geom_histogram() +
  labs(title = "Histogram of life expectancy")

# hist(life_exp$longevity,
...
> It doesn't seem to have any linear relationship, the regression line is probably flat. If Alcohol and life expectancy is independent, then I believe there's not outliers.
...
> It looks like there is some relationship between percentage expenditure and life expectancy but it's curve instead of linear. It seems every variable is good.
...
```{r}
categorical_predictor <- data.frame(Country = c('Cook Islands', 'Kiribati', 'Kiribati', 'Kiribati', 'Kiribati', 'Kiribati'),
                                    Status = c('Developing', 'Developing', 'Developing', 'Developing', 'Developing', 'Developing'))
```

```{r}
# side by side boxplot

#ggplot(data = life_exp, aes(x = continent)) +
